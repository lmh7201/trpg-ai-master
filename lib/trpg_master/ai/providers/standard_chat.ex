defmodule TrpgMaster.AI.Providers.StandardChat do
  ...
  def run(opts) do
    ...
    do_chat_with_retry(
      ...
      Keyword.get(opts, :initial_usage, %{input_tokens: 0, output_tokens: 0}),
      opts
    )
  end

  defp do_chat_with_retry(context, state, retry_count, max_tool_iterations, initial_usage, opts) do
    case do_chat_loop(context, state, max_tool_iterations, initial_usage, opts) do
      {:ok, result} ->
        {:ok, result}
  ...
  defp do_chat_loop(_context, _state, 0, _usage, opts) do
    Logger.warning("#{Keyword.fetch!(opts, :provider)} Tool use 최대 반복 횟수 초과")
    {:error, :max_tool_iterations}
  end

  defp do_chat_loop(context, state, iterations_left, usage, opts) do
    call_api = Keyword.fetch!(opts, :call_api)

    case call_api.(context, state.body) do
      {:ok, response} ->
        usage_info = Keyword.fetch!(opts, :usage_info).(response, usage)
  ...
  defp handle_response(context, state, response, iterations_left, usage, opts) do
    ...
    if response_module.tool_loop?(response) do
      ...
      do_chat_loop(context, updated_state, iterations_left - 1, usage, opts)
    else
      {:ok,
       %{
