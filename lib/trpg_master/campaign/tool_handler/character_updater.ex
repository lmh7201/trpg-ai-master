        ...
        idx ->
          List.update_at(state.characters, idx, fn char ->
            ...
            if target_level > current_level do
              ...
              |> maybe_mark_asi_pending(target_level, char["class_id"], asi, feat)
              |> maybe_mark_subclass_pending(target_level, char["class_id"], subclass)
            else
    ...
    |> then(fn updated ->
      if new_spell_slots do
        ...
        |> Map.update("spell_slots_used", %{}, fn used ->
          Map.merge(
            used,
            Map.new(new_spell_slots, fn {lvl, _} -> {lvl, Map.get(used, lvl, 0)} end)
          )
        end)
      else
        updated
